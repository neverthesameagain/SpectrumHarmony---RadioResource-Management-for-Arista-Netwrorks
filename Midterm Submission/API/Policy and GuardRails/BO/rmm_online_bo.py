...
class PolicyEngine:
    def __init__(self,
        ...
        self.change_window_days = float(change_window_days)
...
def run_online_iteration(csv_path, out_dir, policy=None, sim_time=None):
    """
    Run a single iteration of the online BO loop (one candidate pick/eval per AP).

    Changes (vs your previous function):
    - baseline is taken from the last accepted observation (rolled_back == False)
    - append candidate observation only after post-check accepts it
    - when rejected: log a rejected row (rolled_back=True) and then log a rollback row
      that re-applies the previous config (rolled_back=False)
    - rejected samples are NOT added to GP training data
    """
    ensure_dir(out_dir)
    if policy is None:
        policy = PolicyEngine()
    ...
    try:
        ...
        for ap_id in ap_ids:
            ...
            # ---------------------------
            # Baseline: last accepted observation (rolled_back == False)
            # ---------------------------
            accepted_hist = hist_ap[hist_ap.get('rolled_back', False) == False]
            if not accepted_hist.empty:
                baseline = float(accepted_hist.sort_values('timestamp_start').iloc[-1]['objective_y'])
            else:
                # Fallback: use the last observed objective (if no explicit accepted row exists)
                # If even that is missing, fall back to 0.5 safe default
                try:
                    baseline = float(hist_ap.sort_values('timestamp_start').iloc[-1]['objective_y'])
                except Exception:
                    baseline = 0.5

            # defensive: ensure baseline is finite & non-zero (avoid divide-by-zero checks later)
            ...
            # ---------------------------
            # Current config (last observed row)
            # ---------------------------
            ...
            # median context for candidate evaluation / GP building
            ...
            # choose top EI candidate that passes pre-checks
            for idx in order:
                ...
                # simulate predicted objective (simulator uses median_context)
                ...
                # accept this candidate for evaluation
                selected = cand
            ...
            # Realistic application + measurement (can be simulated); forward sim_time if provided
            if sim_time is None:
                res = simulate_apply_and_measure_realistic(selected)
            else:
                res = simulate_apply_and_measure_realistic(selected, sim_time=sim_time)

            y_obj = res.get("objective_y", None)
            throughput = res.get("throughput_mbps", None)
            rtt = res.get("rtt_ms", None)
            retries = res.get("retries_pct", None)

            # --- POST-CHECK: decide whether to accept or rollback BEFORE writing candidate row ---
            accepted, action_msg = policy.post_check_observation(ap_id, baseline, y_obj)

            if not accepted:
                # --- Rejected: log rejected candidate (rolled_back = True), then re-apply previous config and log rollback row ---
                print("⚠️ Policy post-check failed:", action_msg, "- rolling back to previous config")
                ...
                rejected_row = {
                    ...
                    'num_clients': int(last_row.get('num_clients', 1)),
                    ...
                    'objective_y': float(y_obj) if (y_obj is not None) else None,
                    'rolled_back': True,
                    'policy_msg': f"Rejected: {action_msg}"
                }
                append_row_to_csv(csv_path, rejected_row)

                # Simulate rollback by re-applying the previous config (current_cfg)
                if sim_time is None:
                    rollback_res = simulate_apply_and_measure_realistic(current_cfg)
                else:
                    rollback_res = simulate_apply_and_measure_realistic(current_cfg, sim_time=sim_time)
                ...
                rollback_row = {
                    ...
                    'cos_time': median_context['cos_time'],
                    'ap_id': ap_id,
                    'channel_width_mhz': current_cfg['channel_width_mhz'],
                    'tx_power_dbm': current_cfg['tx_power_dbm'],
                    'obss_pd_dbm': current_cfg.get('obss_pd_dbm', median_context.get('obss_pd_dbm', OBSS_PD_MAX)),
                    'num_clients': int(last_row.get('num_clients', 1)),
                    'client_median_throughput_mbps': rollback_res.get('client_median_throughput_mbps', None),
                    'client_median_rtt_ms': rollback_res.get('client_median_rtt_ms', None),
                    'sensing_cca_busy_pct': median_context['sensing_cca_busy_pct'],
                    'aggregate_neighbor_load_pct': median_context['aggregate_neighbor_load_pct'],
                    'client_video_stalls_frac': 0.0,
                    'client_retries_pct': rollback_res.get('client_retries_pct', None),
                    'window_minutes': WINDOW_MIN,
                    'objective_y': float(rollback_res.get('objective_y', np.nan)),
                    # rollback row represents re-applied accepted state
                    'rolled_back': False,
                    'policy_msg': "Rollback applied"
                }
                append_row_to_csv(csv_path, rollback_row)

                # do NOT add rejected sample to GP training set; skip to next AP
                continue

            # accepted -> append the real observation into CSV (rolled_back False)
            new_row = {
                'timestamp_start': datetime_now().isoformat(),
                'timestamp_end': (datetime_now() + timedelta(minutes=WINDOW_MIN)).isoformat(),
                'sin_time': median_context['sin_time'],
            ...
            print("✅ Candidate accepted; logged simulated observation.")

            # --- Retrain GP with new data (only for accepted observation) ---
            widths = CHANNEL_WIDTHS
            ...
            models[ap_id]['y_used'] = np.concatenate([models[ap_id]['y_used'], np.array([float(y_obj) if y_obj is not None else np.nan])])
            # refit GP (if y_obj is nan, fit will likely blow up — but we keep the same behavior as prior code)
            try:
                models[ap_id]['gpr'] = fit_gp(models[ap_id]['X_used'], models[ap_id]['y_used'])
            except Exception as e:
                print("GP retrain failed after accepted sample:", e)
